...
const CHUNK_UPLOAD_TIMEOUT = 60000; // 60 seconds
const FILE_ASSEMBLY_TIMEOUT = 180000; // 180 seconds
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
            ...
            if (error.name === 'AbortError') {
                timeoutCount++;
                console.warn(`Chunk upload timed out on attempt ${attempt + 1} after ${CHUNK_UPLOAD_TIMEOUT / 1000} seconds`);
...
export async function uploadFiles(files, endpoint, formDataExtras = {}, onProgress = () => {}) {
    ...
    for (const file of files) {
        ...
        try {
            ...
            const controller = new AbortController();
            const timeoutId = setTimeout(() => controller.abort(), FILE_ASSEMBLY_TIMEOUT);
